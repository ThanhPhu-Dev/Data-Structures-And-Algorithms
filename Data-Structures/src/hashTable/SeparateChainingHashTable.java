...
public class SeparateChainingHashTable<K,V> implements HashTableADT<K, V> {
    ...
    public SeparateChainingHashTable(double loadFactor, int capacity) {
        if(capacity < 0) throw new IllegalArgumentException("Capacity không âm");
        if(loadFactor <= 0 || Double.isNaN(loadFactor) || Double.isInfinite(loadFactor)) {
            throw new IllegalArgumentException("load factor không âm");
        }
    ...
    /**
     * @param hashedKey hashcode
     * @return vị trí tương ứng hashcode trong mảng
     */
    @Override
    ...
    public V insert(K key, V value) {
        ...
        if(linkedList == null ) {
            ...
            //kiểm tra value đó có tồn tại trong linked không nếu có thì chỉ cập nhập value cho key đó và trả về value củ
            for(Node<K,V> node : linkedList) {
            ...
            //nếu không bị trùng lập thì thêm vào linked vị trí cuối cùng.
            return addNodeToBucket(key, value, linkedList);
        }
    }
    ...
    public Iterator<K> iterator() {
        ...
        return new Iterator<K>() {
            ...
            public boolean hasNext() {
                if(elementCount != size()) throw new ConcurrentModificationException("Table bị thay đổi!");

                //nếu linkedlist đó là null hoặc isEmpty thì sẽ chuyển qua linked tiếp theo
                if(bucketIterator == null || !bucketIterator.hasNext()) {
                    while(++index < capacity) {
                        //nếu linkedlist đó có tồn tại thì break
                ...
                //nếu không có linked nào có data thì index sẽ chạy vượt qua capacity
